...
| **Phase 3** | 数据浏览核心    | ✅ 已完成 | Key 浏览器 + 7 种数据类型增删改查（含 RedisJSON）+ TTL 管理 + 批量操作 + 收藏 + 多格式值编辑器 |
...
| #   | 任务                       | 说明                                                                    | 状态 |
| --- | -------------------------- | ----------------------------------------------------------------------- | ---- |
| 10  | String 值查看/编辑器       | 纯文本编辑 + JSON 高亮/格式化 + Hex 视图                                | ✅   |
...
| 22  | 新建 Key       | 对话框选择类型并联动初始值输入，复用多格式值编辑器                             | ✅   |
| 23  | 重命名 Key     | 后端命令 + 详情面板操作按钮                                                    | ✅   |
| 24  | 复制 Key       | 后端 `copy_key` + 前端详情面板“更多”菜单入口                                   | ✅   |
| 25  | 删除 Key       | 单个删除 + 批量删除，统一危险确认弹窗，删除后局部更新列表                      | ✅   |
...
| #   | 任务                | 说明                                                                        | 状态 |
| --- | ------------------- | --------------------------------------------------------------------------- | ---- |
| 28  | 集成内置值编辑器    | 使用原生 textarea + JSON 高亮叠层，保留原生输入和 IME 行为                  | ✅   |
| 29  | 多格式显示/编辑     | 支持 Text / JSON / XML / YAML / HTML / CSS / JS / TS / SQL / Markdown / Hex | ✅   |
| 30  | JSON 校验与格式化   | JSON 模式实时校验，错误定位到行列，支持一键格式化                           | ✅   |
| 31  | 格式自动检测        | 自动检测值的格式并切换对应语言模式                                          | ✅   |
| 32  | Hex dump 视图       | 只读 Hex dump + 256KB 截断提示，避免大值渲染卡顿                            | ✅   |
| 33  | 值内搜索            | 文本区域保留浏览器原生搜索与选择能力                                        | ✅   |
| 34  | 主题跟随            | JSON 高亮和编辑器样式跟随应用主题切换                                       | ✅   |
| 35  | 复制到剪贴板        | 一键复制 Key 名称或值                                                       | ✅   |
| 36  | 表格交互重构        | Hash/List/Set/ZSet 表格行值截断单行 + Tooltip + 单击展开 + 双击编辑弹窗     | ✅   |
| 37  | 新建 Key 初始值编辑 | 新建弹窗按类型联动 field/member/score/position，并复用多格式编辑器          | ✅   |
| 38  | 大字符串性能优化    | Hex useMemo 缓存 + 256KB 截断 + 大值预览/完整加载                           | ✅   |
...
- ✅ 7 种数据类型的查看与编辑（String/Hash/List/Set/ZSet/Stream/JSON）
- ✅ Key 管理操作（新建/重命名/删除/TTL）
- ✅ 值编辑器（原生 textarea + JSON 高亮叠层 + 多格式切换 + Hex dump + 主题跟随）
- ✅ 连接导入导出后端命令
...
- ✅ 新建 Key 类型联动初始值输入（String/Hash/List/Set/ZSet/Stream/JSON）
- ✅ 大字符串性能优化（Hex useMemo + 256KB 截断 + 大值预览/完整加载）
...
| #   | 任务           | 状态 | 说明                                                                                                                                                |
| --- | -------------- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| 13  | 全局快捷键     | ✅   | ⌘N（新建连接）/ ⌘T（CLI）/ ⌘F（搜索）/ ⌘R（刷新）/ ⌘K（命令面板）/ ⌘D（删除选中 Key）/ ⌘S（保存编辑）/ ⌘,（设置）/ F5（刷新）/ Delete（删除选中项） |
| 14  | 输入框辅助关闭 | ✅   | 全局关闭自动大写、自动纠错和拼写检查，避免 Key 名和值编辑被系统输入辅助改写                                                                         |
| 15  | Toast 通知系统 | ✅   | 已迁移至 sonner（连接成功/失败等）                                                                                                                  |
| 16  | 错误边界       | ✅   | ErrorBoundary 组件：捕获渲染异常 + 展示降级 UI + 错误详情                                                                                           |
| 17  | 加载状态       | ✅   | 已在各 Store 中管理 loading 状态                                                                                                                    |
| 18  | 敏感操作确认   | ✅   | ConfirmDangerDialog：删除、批量删除、FLUSHDB/FLUSHALL 等危险操作统一确认                                                                            |
