...
| 30 | Diff 对比 | 使用 Monaco Diff Editor 展示编辑前后差异 | ✅ |
...
- ✅ 连接导入导出后端命令
- ✅ 大值延迟加载（> 1MB 先预览，点击完整加载）
- ✅ 批量操作工具栏（多选 + 批量删除 + 批量导出）
- ✅ 收藏 Key（⭐标记 + 持久化 + 仅显示收藏筛选）
