...
| 10 | 连接分组（文件夹） | ✅ | 支持分组管理：对话框中设置分组、侧边栏按组折叠展示、右键菜单移动分组 |
...
- ✅ 连接分组管理（分组创建、移动、折叠/展开，拖拽排序待后续迭代）
...
| 18 | 大值延迟加载 | 值 > 1MB 时先显示摘要，点击后完整加载 | 🔲 延后 |

#### 3.3 Key 管理操作

| # | 任务 | 说明 | 状态 |
|---|------|------|------|
| 19 | Key 信息面板 | 展示类型、TTL、大小、编码方式 | ✅ |
| 20 | TTL 管理 | 设置/修改/移除 TTL 的 UI 和后端命令 | ✅ |
| 21 | 新建 Key | 对话框选择类型并输入初始值 | ✅ |
| 22 | 重命名 Key | 后端命令 + 详情面板操作按钮 | ✅ |
| 23 | 复制 Key | 后端通过 `COPY` 命令实现 | ✅ |
| 24 | 删除 Key | 单个删除 + 批量删除，二次确认 | ✅ |
| 25 | 批量操作工具栏 | 多选 Key 后显示批量操作按钮 | 🔲 延后 |
| 26 | 收藏/标记 Key | 常用 Key 标记，快速访问 | 🔲 延后 |

#### 3.4 值编辑器增强（基于 Monaco Editor）

| # | 任务 | 说明 | 状态 |
|---|------|------|------|
| 27 | 集成 Monaco Editor | 使用 `@monaco-editor/react` 作为核心值编辑器 | ✅ |
| 28 | 多格式语法高亮 | 支持 JSON / XML / 纯文本，自动检测 | ✅ |
| 29 | 格式自动检测 | 自动检测值的格式并切换对应语言模式 | ✅ |
| 30 | Diff 对比 | 使用 Monaco Diff Editor 展示编辑前后差异 | 🔲 延后 |
| 31 | 值内搜索 | 大文本中支持 Ctrl+F 搜索（Monaco 内置） | ✅ |
| 32 | 主题跟随 | 深色/浅色主题自动跟随应用主题切换 | ✅ |
| 33 | 复制到剪贴板 | 一键复制 Key 名称或值 | ✅ |
...
- [x] 连接分组（文件夹）— 支持分组展示、创建分组、右键移动分组
- [ ] 连接分组拖拽排序 — 拖拽连接在分组间移动、排序
