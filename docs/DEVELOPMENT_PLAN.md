...
- [x] 补充快捷键 — ⌘D（删除选中 Key）/ ⌘S（保存编辑）/ F5（刷新 Key 列表）/ Delete（删除选中项）

### 自动更新（Phase 6）

- [x] 集成 Tauri Updater Plugin — `@tauri-apps/plugin-updater` + 签名密钥 + 前端 API 封装
- [x] 更新检查逻辑 — 启动时检查 + 可配置检查频率（24h 间隔 + 开关控制 + 手动检查）
- [x] 更新提示 UI — 发现新版本弹窗（版本信息 + 更新说明 + 下载进度 + 安装/重启）
