...
本项目是基于 Tauri 2 和 Next.js 16 的 Redis 桌面客户端。前端源码位于 `src/`：`app/` 存放 App Router 入口，`components/` 存放功能组件和 shadcn/ui 组件，`hooks/` 存放通用 React Hooks，`stores/` 存放 Zustand 状态，`lib/` 存放工具函数与 Tauri API 封装，`i18n/` 存放国际化配置。Rust 后端位于 `src-tauri/src/`，按 `commands/`、`redis/`、`config/` 分层。静态资源在 `public/`，项目文档在 `docs/`。
