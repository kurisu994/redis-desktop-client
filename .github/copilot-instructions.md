...
│   └── layout/            # 布局组件（sidebar, titlebar, status-bar, welcome-page）
├── stores/                # Zustand 状态仓库
...
使用 **Conventional Commits**，由 commitlint + husky 强制执行。
...
refactor(store): 重构连接状态管理逻辑
docs(readme): 更新安装说明
```

**注意**：
- 提交信息全部使用中文
- **不要**加 emoji 前缀（commitlint 会拒绝）
- **不要**添加 `Co-authored-by: Copilot` trailer
- type 可选值：feat, fix, docs, style, refactor, perf, tests, chore, build, CI, wip

## 设计参考

`docs/` 目录包含设计稿图片：
- `1.png` — 欢迎页（空状态，无连接时展示）
- `2.png` — 新建连接对话框
- `3.png` — 已连接视图（键浏览器）
